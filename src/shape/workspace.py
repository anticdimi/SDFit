...
# ! Code adapted from https://github.com/ZhengZerong/DeepImplicitTemplates/blob/master/networks/deep_implicit_template_decoder.py
...
from .dit_decoder import Decoder
...
def load_decoder(experiment_dir: Path | str, checkpoint, device: str = 'cuda') -> 'Decoder':
...
def load_pretrained_dit_latents(
    exp_dir: str, checkpoint: int, device: str = 'cuda'
) -> list[torch.Tensor] | torch.Tensor:
    from pathlib import Path

    filename = Path(exp_dir) / 'LatentCodes' / f'{checkpoint}.pth'

    if not filename.exists():
        raise Exception(
            f'The experiment directory ({exp_dir}) does not include a latent code file'
