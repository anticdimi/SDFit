# ! Code is adapted from https://github.com/niladridutt/Diffusion-3D-Features
import math
...
from pytorch3d.renderer.mesh.rasterizer import Fragments, MeshRasterizer, RasterizationSettings
...
def run_rendering(
    ...
    if steps * steps < num_views:
        steps += 1
    end = 360 - 360 / steps
    elevation = (
        torch.linspace(start=0, end=end, steps=steps).repeat(steps)[:num_views] + add_angle_ele
    )
    azimuth = torch.linspace(start=0, end=end, steps=steps)
    azimuth = torch.repeat_interleave(azimuth, steps)[:num_views] + add_angle_azi
...
def batch_render(device, mesh, num_views, H, W, use_normal_map=False):
    trials = 0
    add_angle_azi = 0
    add_angle_ele = 0
    while trials < 5:
