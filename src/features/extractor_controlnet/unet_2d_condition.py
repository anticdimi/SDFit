...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        is_adapter = (
            mid_block_additional_residual is None and down_block_additional_residuals is not None
        )
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            # ! Extract features from the UNet
            if return_features and (i == 1 or i == 3):
                f_map[f'unet_fmap_{i+1}'] = sample[0].unsqueeze(0).cpu().clone()
                # f_map = sample[0].unsqueeze(0).cpu().clone().float()
                # f_map.append(sample[0].unsqueeze(0).cpu().clone())
                # f_map = sample[::2].mean(dim=0).unsqueeze(0).clone()
                # f_map = torch.cat((sample[0], sample[1]), dim=0).unsqueeze(0)
                # f_map = torch.cat((sample[0], sample[1]), dim=0).unsqueeze(0)
                # return sample #((sample[0]+sample[1])/2).unsqueeze(0)
