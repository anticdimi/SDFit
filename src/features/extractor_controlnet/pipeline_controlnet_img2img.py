...
class StableDiffusionControlNetImg2ImgPipeline(
    ...
    def prepare_latents(
        ...
        if batch_size > init_latents.shape[0] and batch_size % init_latents.shape[0] == 0:
            ...
            deprecate(
                'len(prompt) != len(image)', '1.0.0', deprecation_message, standard_warn=False
            )
    ...
    def __call__(
        ...
        torch.linspace(0.1, 1, steps=aggregation_steps).to(device)
        # f_map_weights = (f_map_weights - f_map_weights.min()) / (f_map_weights.max() - f_map_weights.min())
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #     f_map += (
                #         torch.nn.functional.normalize(noise_pred[1], dim=1)
                #         * f_map_weights[f_map_iters]
                #     )
                #     f_map_iters += 1
                # if extract_last_only and extract_layer:
                #     f_map = torch.nn.functional.normalize(noise_pred[1], dim=1)
                # if i == (num_inference_steps - 1) and not return_image:
                #     return StableDiffusionPipelineOutput(
                #         images=f_map[0], nsfw_content_detected=[True]
                #     )
                # noise_pred = noise_pred[0]

                if return_features:
                    if isinstance(noise_pred[1], dict):
                        for key, value in noise_pred[1].items():
                            noise_pred[1][key] = torch.nn.functional.normalize(value.float(), dim=1)
                        # noise_pred = noise_pred[0]
                        f_map = noise_pred[1]
                    else:
                        f_map = torch.nn.functional.normalize(noise_pred[1], dim=1)[0]
                if i == (num_inference_steps - 1) and not return_image:
                    return StableDiffusionPipelineOutput(images=f_map, nsfw_content_detected=[True])
        ...
        return StableDiffusionPipelineOutput(
            images=[f_map, image], nsfw_content_detected=has_nsfw_concept
        )
